...
pub fn process_collect(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    // Load accounts.
    let [signer_info, boost_info, boost_config_info, boost_proof_info, boost_rewards_info, config_info, proof_info, rewards_info, stake_info, treasury_info, treasury_tokens_info, ore_program, ore_boost_program, token_program] =
        accounts
    else {
    ...
    rewards_info
        .as_associated_token_account(&config_info.key, &ore_promo_api::consts::NFT_MINT_ADDRESS)?;
    ...
    invoke_signed(
        &ore_boost_api::sdk::claim(
            *config_info.key,
            *rewards_info.key,
            ore_promo_api::consts::NFT_MINT_ADDRESS,
            amount,
        ),
        &[
            config_info.clone(),
